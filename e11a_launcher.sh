#!/usr/bin/env bash
# E1.1: elastic-canvas fine-tune from the migrated official OmniVoice ckpt.
set -euo pipefail
...
exec accelerate launch --gpu_ids "${GPU_IDS}" --num_processes "${NUM_GPUS}" \
    ...
    --train_config examples/config/train_config_elastic_e11a.json \
    --data_config examples/config/data_config_emilia_yodas_exp.json \
    --output_dir "${OUTPUT_DIR:-exp/elastic_e11a}" 2>&1 | tee -a logs/e11a_train.log
