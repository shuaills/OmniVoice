...
gen_lang() {
  ...
  for i in $(seq 0 $((NGPU-1))); do
    CUDA_VISIBLE_DEVICES=$i python tests/seedtts_blockwise_gen.py \
      ...
      --shard ${i}/${NGPU} >> "logs/b2g50k_nanfix_gen_${lang}_shard${i}.log" 2>&1 &
    pids+=($!)
  done
...
for lang in zh en; do
  ...
  if python omnivoice/eval/wer/seedtts.py \
      ...
      --decode-path "${RES}/wer_${lang}.tsv" --batch-size 4 > "logs/b2g50k_nanfix_wer_${lang}.log" 2>&1; then
    AVG=$(grep -oE "Seed-TTS WER \(Avg of WERs\): [0-9.]+%" "logs/b2g50k_nanfix_wer_${lang}.log" | tail -1 | grep -oE "[0-9.]+%")
    WTD=$(grep -oE "WER \(Weighted\): [0-9.]+%" "logs/b2g50k_nanfix_wer_${lang}.log" | tail -1 | grep -oE "[0-9.]+%")
...
for lang in zh en; do
  ...
  if python omnivoice/eval/speaker_similarity/sim.py \
      --wav-path "${RES}/seedtts_${lang}" --test-list "${RES}/test_${lang}.jsonl" \
      --model-dir "${MODELS}" \
      --decode-path "${RES}/sim_${lang}.tsv" > "logs/b2g50k_nanfix_sim_${lang}.log" 2>&1; then
    S=$(grep -oE "SIM-o score: [0-9.]+" "logs/b2g50k_nanfix_sim_${lang}.log" | tail -1 | grep -oE "[0-9.]+")
...
if CKPT="${CKPT}" NPROMPTS=12 CUDA_VISIBLE_DEVICES=0 \
    python tests/eos_displacement_test12.py > "logs/b2g50k_nanfix_eos12.log" 2>&1; then
  {
    echo "--- junction raw table (ban=0/8/30) ---"
    grep -E "ban=0:" "logs/b2g50k_nanfix_eos12.log"
  } >> "${VF}"
  STOPS=$(grep -E "ban=0:" "logs/b2g50k_nanfix_eos12.log" | grep -oE "ban=0: T=[0-9]+" | grep -oE "[0-9]+$" | awk '$1 < 32 {c++} END {print c+0}')
  TOTAL=$(grep -cE "ban=0:" "logs/b2g50k_nanfix_eos12.log")
