#!/usr/bin/env bash
# B2F: B2 recipe + edge-filler data filter (single-variable rerun from same init).
set -euo pipefail
...
exec accelerate launch --gpu_ids "${GPU_IDS}" --num_processes "${NUM_GPUS}" \
    ...
    --train_config examples/config/train_config_block_b2f.json \
    --data_config examples/config/data_config_emilia_yodas_exp.json \
    --output_dir "${OUTPUT_DIR:-exp/block_b2f}" 2>&1 | tee -a logs/b2f_train.log
