#!/usr/bin/env bash
# B1: block-diffusion conversion fine-tune from the migrated official ckpt.
set -euo pipefail
REPO_ROOT="/opt/gpfs/users/shuai/work/block-conversion/OmniVoice"
...
exec accelerate launch --gpu_ids "${GPU_IDS}" --num_processes "${NUM_GPUS}" \
    ...
    --train_config examples/config/train_config_block_b1.json \
    --data_config examples/config/data_config_emilia_yodas_exp.json \
    --output_dir "${OUTPUT_DIR:-exp/block_b1}" 2>&1 | tee -a logs/b1_train.log
