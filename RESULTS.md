...
## 2026-07-09 深夜 — B2G-50k 全集判决（shuai-b2g-eval 8 卡 30 分钟跑完）+ 尾部法证

**主表行（steps16/gs2.0/fp16 锁定配置）**：

| 指标 | B2(YODAS) | B2G 全量 | B2G 剔截断 | 官方 |
|---|---|---|---|---|
| zh WER | 4.16 | 5.69 | **3.50** | 0.89 |
| en WER | 3.68 | 11.05 | **3.76** | 1.65 |
| zh SIM | 0.693 | 0.673 | 0.689 | 0.778 |
| en SIM | 0.641 | 0.630 | 0.649 | 0.741 |

**法证链（判决级）**：
1. en 11.05 / zh 5.69 全部由 **瞬停截断尾巴** 驱动：100% WER 失败例 zh 46/2020、en 28/1088，其中 frames<32（首块内 EOS）zh 41、en 22 —— **双语一致 ~2% 出生即死率**；剔除 frames<64 后幸存集 zh 3.50（**优于 B2 4.16，−16%**）、en 3.76（持平 3.68）。en 均值更敏感因短真值 + Avg-of-WERs。
2. **SIM 判决：官方同源数据没有移动 SIM**（剔截断 0.689/0.649 vs B2 0.693/0.641，平）→ SIM 缺口的"数据域差"假设死亡，**转换训练天花板确认为约束**。
3. 全集口癖 first-100 = 30/100（ASR 地板判据；用户耳测"多数整首块"）→ 与探针 23/100 同量级。
4. 全集 junction 探针 = 2/12（与 dev3 探针一致，复现）。
5. **统一病理**：截断尾巴（eval headline 杀手）、首块口癖（EOS 被压制时的位移）、句末残差异响（终块提交）= 同一 junction/EOS 终止先验的三种表达。官方数据修不动它（第三数据制度确认）；它也是主表与官方差距的最大单一来源。
6. 下一步已在跑：逆镜像 A/B（推理侧去 seam 标点，dev3 4090，分支 probe/inference-text-trainlike-20260709）——若截断/口癖坍缩，则 (a) 零训练即可回收 headline（SEAM_PUNCT=off 重跑全集），(b) B2H/3b 训练侧根修近乎必中。

## 2026-07-09 深夜 II — 逆镜像 A/B 判决：text-seam 假设死亡，B2H（原设计）取消建议

- **空转发现**：`add_punctuation` 对 zh Seed-TTS 全部 2020 条 prompt 是 no-op（原转写本就带句末标点）；确定性对照 12/12 逐字节复现（2/12 瞬停原样）。
- **strip A/B**（去掉本就存在的句末标点，单变量）：junction 2/12 不变（同两条，标点无感）；**口癖 23→36/100（+57%）；WER-100 4.69→7.85（+67%）**——模型对 seam 文本格式**有强因果敏感度**，但自然格式已是较优设定，且带病地板。
- **数据普查（判决级）**：内部 436kh zh 转写 **99.4% turn 尾带标点**（4256/4280）——训练文本与推理拼接文本在边界处字节等价 → **训练/推理文本失配假设死亡**；**杠杆 3a 镜像 = 近似恒等变换，B2H 原设计不再值得跑**（今晚零训练探针链救下一次会被误读的 null 实验）。
- **剩余嫌疑人**（2% 瞬停尾巴 + 口癖地板）：① NAR init 继承（用户假设）——**B2S-30k 表型探针已在 4090 上跑**；② EOS 标签设计残留（4 列窗仍教终止区先验）；③ prompt 音频侧终止韵律（干净收尾+尾静音的 eval 风格 prompt 触发终止判断——10003502 从未被任何配方治愈，优先查它的 prompt 波形）。
- 工件：perf 克隆分支 probe/inference-text-trainlike-20260709（9ff99da, bc7d0c2）；strip 条件 100 wav 已拉用户本地（zh_first100_seampunct_strip/）。

## 2026-07-09 深夜 III — junction/EOS 战场四连判决：NAR-init 假设裁决 + 零尾巴治愈 + fillerometer 仪器上线

**① B2S-30k 表型探针（从头模型，6% 训练量，dev3 4090）**
- junction 12-prompt（ban=0）：**1/12** —— 10003502-00000044（历任转换配方从未治愈、B2G T=0）从头 **T=189 痊愈** → 该瞬停是转换/init 携带；10002430-00000015 双 init 都 T=1 → init 无关（见 ③）。其余 10 条全干净 T=101–194，lead0=0 全场。
- **素面克隆零截断**：100/100 wav、EOS 100/100、frames min 53 / 中位 166.5 / max 303，**frames<32 = 0 条**（<64 仅 1 条）——对照 B2G 全集 ~2% 出生即死 → **瞬停截断尾巴 = 转换携带病**，从头模型根本没有。
- WER-100 = 20.83%（0 空转写、仅 1 条 <4 字——6% 训练量的真实幼稚语音，非哑火，读数可解释）。
- **口癖判决（NAR-init 假设证伪）**：ASR 判据 55/100（B2G 23/100，反而更糟）；**fillerometer 声学读数 84/100 ≈ B2G 88/100**（阈值扫描 0.15–0.40 → 86/84/84/80/71，稳健）→ 双 init 同处 85–90% 饱和区。**哼鸣不是 NAR ckpt 继承，是配方几何自己长出来的**；ASR 计数差异只说明从头模型的哼更响/更元音化（paraformer 更常听成呃/啊）。100 条音频已拉用户本地 ~/Desktop/b2s_30k_audio/zh_first100/。
- 操作性发现（复用）：B2S ckpt 存的 config.json 陈旧（llm_config.vocab_size=151936，权重实为 151676 已 resize）→ 直接加载报 embed_tokens 尺寸错。解法 = shim ckpt 目录（symlink safetensors + 单字段补丁，results_b2s30k_probe/shim_ckpt/）。每次 B2S eval 都需要，或在保存侧修 config。

**② fillerometer v1（新仪器，耳测验证，判决级）**
- 特征：首 1.28s 窗，25ms 帧 / 10ms 跳；active = rms > max(峰值×0.15, 1e-4)；voiced = 自相关 60–350Hz 滞后带峰 > 0.5；**score = active_frac × (1 − voiced_frac)**，哼鸣阈 0.25。脚本在用户 Mac ~/.claude/jobs/5877acf0/tmp/fillerometer_v1.py。
- 验证：B2G 23/23 ASR 命中全召回；用户耳测干净 2 条得分 0.00 / 0.09。
- 读数：B2G-50k **88/100**（ASR 23% 是地板——paraformer 直接吞掉非词汇哼鸣）；strip 条件 83/100（发生率饱和；strip 恶化的是哼鸣**时长**——v2 run-length 指标待做）。
- 声学画像 + 机理：哼鸣 = 10–15Hz 振幅脉冲串 + 低频谐波，对自相关呈弱周期（所以 v0 的 voiced 特征反相，v1 修正）。**自激机制**：哼鸣块进入干净 KV 历史 → 下一块续哼（跨块级联，解释 strip 条件的多块 ememem）。

**③ 零尾巴发现 + padding 微实验（判决：治愈，双 init）**
- 法证：10002430-00000015 的 ref 是 100 条中唯一 trailing_silence = 0.000s（其余中位 0.13s、最长 0.57s）——init 无关的死点 = 声学上唯一异常的 prompt。
- A/B（ban=0；PROMPT_FILTER + TAIL_PAD_S 环境旗，perf 克隆探针分支 commit 5090751；tail_pad 激活行单变量确认；对照精确复现 T=1）：

| ckpt | control (0s) | +0.15s | +0.30s |
|---|---|---|---|
| B2G-50k（转换） | T=1 | **T=162** | T=135 |
| B2S-30k（从头） | T=1 | **T=155** | T=116 |

- **0.15s 数字静音把双 init 下永不治愈的死点翻成全长生成**。记录在案：B2G+0.15 在 ban=8/30 窗下 T=14（padding 不是强制续写窗的万能护栏；ban 窗非预注册读数）。
- 含义：推理侧 **prompt 尾静音归一化（≥0.15s）= 合法输入规范**（AR 世界 ref 音频规范同款，F5-TTS 等有同类建议）；训练侧根修（模型该学会"零尾巴 ≠ 终止"）仍列下一仗。

**④ 病理分解总表（全部实证，判决级）**
| 表型 | 发生率 | 来源 | 证据 |
|---|---|---|---|
| 瞬停截断（系统性） | B2G 全集 ~2%，双语一致 | **转换/init 携带** | B2S 素面克隆 0 条；10003502 从头痊愈 |
| 瞬停（prompt 型） | 单例（10002430） | **prompt 零尾静音** | +0.15s 治愈双 init |
| 首块哼鸣 | 双 init 85–90%（声学） | **配方几何 / 数据 onset 先验**（探针在跑） | fillerometer 84 vs 88；三数据制度不变 |
...
## 2026-07-10 凌晨 — rescue 判决：min_gen_frames 一行修复零训练回收 headline + fillerometer v2 严重度仪器

**① rescue 重跑判决（63 条截断 utt × 修复采样器 4bf82df，4090）**
- **zh：42 条截断 → 仅 1 条仍 <32**（10002394-00000011，24→26，其 floor=24 = 合法短停）；3 条 <64。**fix_failed = 0。**
- **en：25 条截断 → 11 条仍 <32，全部合法**（短 Common Voice 目标的 floor=10–27，每条停点 ≥ 自己的 floor）。en 早停引力仍在但已被 floor 按设计兜住；放行者是 0.3×估计系数本身（**设计问题非 bug**——en 短文本的 floor 太低）。
- 新 frames 分布：zh min/med/max = 26/181.5/249（原 2/18.5/31，整体复活）；en 11/38/162（双峰：14 条全长 89–162 + 11 条合法短停）。
- **headline 拼接（判决级）**：

| | 原全集 | 修复拼接后 | B2(YODAS) | 官方 |
|---|---|---|---|---|
| zh WER | 5.69 | **3.83** | 4.16 | 0.89 |
| en WER | 11.05 | **7.06** | 3.68 | 1.65 |

- **zh 全集 3.83 实测优于 B2 4.16 —— 零训练、一行采样器修复兑现**（此前"剔截断 3.50"是估计，这次是真跑）。rescued 子集 WER 99.8%→10.4%（39/42 出真语音，中位 ~5%，多条 0.0）。
- en 7.06 保守：227.9%→54.3%（旧值含一条 2 帧剪辑的 whisper 幻觉 3511% 离群）；残差集中在 11 条合法短停（~100%）。两条 en 边缘 utt（17288057/18466098）旧 0.0 新 100 = 短目标的 run-to-run 方差，拼接对我们不利 ~0.4pt，照记。
- 工程备忘：4090 跑 whisper-large-v3 WER 需 --batch-size 4（默认 16 会 ~22GB OOM）；paraformer 默认即可。
- 工件：perf 克隆 results_trunc_rescue/{zh,en}/、wer_{zh,en}.tsv、rescue_run.sh、logs/rescue_*。
- **含义链**：转换携带的早停引力（B2S 没有）+ seed_rem 计数 bug（护栏漏气）= 2% 出生即死。修 bug 后护栏兜住引力 → headline 回收。根修引力本身仍是训练侧课题（EOS 先验），但 eval/serving 即刻受益。**建议：批准全集 re-eval（8 卡 30 分钟）以正式表格取代拼接值；主树采样器修复待 B2S 安全窗。**

**② fillerometer v2（严重度仪器上线，Mac tmp/fillerometer_v2.py）**
- 指标：lead_hum_s = 语音 onset（首个 ≥0.25s 持续浊音段）之前的"活跃且非浊"帧总时长；同报 lead_sil_s / onset_s / v1 兼容分。
- 耳测锚：两条耳测干净对照 lead_hum=0.00s（前导为 0.68/0.72s 干净静音）——完美分离。
- 读数：B2G 基线 lead_hum 均值 0.71s / 中位 0.66s / p90 1.54s / 最长 3.9s；token-dump 6 条极端例 1.65s 均值（≈token-0 块数，严重度与 token 签名直接挂钩）；B2S 0.35s 均值（更短但更"元音化"）。
- **"strip 加长哼鸣"假设未获确认**：配对 delta 均值 +0.02s / 中位 −0.02s（28 恶化 46 改善）——strip 的 WER 恶化（+67%）另有来源。仪器已知盲区：**浊化的呃（词汇化 filler）逃逸本指标**（它是浊音）——v1 管发生率、v2 管非浊哼鸣时长、ASR 管词汇化 filler，三仪器合围。注意 v2 的 v1 兼容分归一化不同（rms.max 取 4s 窗），发生率读数以 v1 原版为准（88/100）。

**③ P2 token-0 onset ban 探针已开跑**（ONSET_BAN_BLOCKS=1 @2a3a158）：junction-12 + 6 utt token 转移检查（看 token 0 被谁接替：244 静音族=治愈 / 其他单一罕见 token=吸引子转移 / 多样=真内容）+ gen-100 + WER/filler/声学三仪器。

## 2026-07-10 凌晨 II — 哼鸣终审：CFG NaN bug（推理侧），非模型病理。WER-100 4.69→0.23%、口癖 23→0/100、声学哼鸣 88→4/100

**根因链（逐步实证，用户"很可能是我们哪里搞出来 bug 了"直觉三连中）**：
1. eval 生成路径 min_gen_frames floor 恒开（max(8, 0.3×est)），EOS ban 把 c_logits **和** u_logits 的 eos 项都写成 −inf；
2. CFG 合成 `c + gs×(c−u)` 在该项算出 **(−inf) − (−inf) = NaN**；
3. `_predict_tokens_blockwise` 外层 log_softmax 把 NaN 传染整行 cb0；
4. **argmax(全 NaN 行) = index 0**（= "哼鸣 token"），且 NaN 置信度在 topk 居首 → 这些位置**最先提交** → 首块 cb0 全 token 0 = 哼鸣 → 进 KV 级联。
**token 0 从来不是模型选的。** 两段修复（探针分支，已推 ysgit）：c336b52 有限 ban 值；9cc16e4 dtype 感知 `torch.finfo(dtype).min` —— **fp16 里 −1e30 溢出回 −inf**，c336b52 单独无效（全零输出逐字节复现验证）。通用军规入库：**凡 pre-softmax/pre-CFG ban 一律用 finfo.min，禁 −inf 与手写大负数**。

**修后读数（B2G-50k，zh first-100，floor 恒开、无 onset ban、激活行 0 次确认）**：
| 指标 | 污染值 | 修后 | 官方参照 |
|---|---|---|---|
| WER-100 | 4.69% | **0.23%**（仅 5 条非零，最大 4.76%） | 全集 0.89% |
| ASR 口癖 | 23/100 | **0/100** | 0.6% |
| 声学哼鸣（fillerometer） | 88/100 | **4/100**（中位分 0.65→0.016） | — |
| frames | 含 2% <32 | 106/159/245，**0 条 <64** | — |
（子集定方向军规仍在：0.23% 是 first-100 读数，全集见 re-eval。残余 4 条声学哼鸣≈官方天性停顿量级，其中 10002702-00000007/10002895-00000015 两条 >0.9 待听。）

**junction 复测（修后有效版 onsetban_junction_valid.log）**：ban=0 仍 **2/12**（10002430 T=1、10003502 T=0）——**真实的转换携带 EOS 引力仍在**（此前 ban=0 诊断从未被 NaN 污染，全部结论成立）；ban=8/30 现在把两个死点顶到全长 **T=154/147 且 lead0=0 全场** —— 护栏首次无伪影工作（此前 ban 窗=强制哼鸣器）。token 转移检查：无转移——修后首块 = 真静音 244 领衔 + 多样内容（0/192 个 token 0）；ONSET_BAN_BLOCKS 与不 ban 逐字节一致 = **no-op，永久搁置**。

**污染清单（判决级，逐条重标）**：
- **作废（floor-on 生成的所有哼鸣/口癖表征）**：B2G 88/100 与 23-30/100；B2S 84/100 与 55/100 及 WER 20.83%（重测中）；strip 口癖/时长读数；"配方放大 3-6×" 结论（放大器=NaN）；"B2S 病更重"；fillerometer 严重度均值（0.71s 描述的是 bug 输出）。
- **成立**：ban=0 junction 全部诊断（2/12、B2S 1/12、零尾巴发现、padding 治愈、10003502 转换携带）；token 审计（数据无 0-run、静音=244 族、无索引移位）；数据 onset 先验 28%（直接 vocode 数据 token，不经采样器）；截断法证与 seed_rem 修复（回归测试 5/5）；vocoder 无罪；fp32 注意力战役全部。
- **下修为下界**：rescue 拼接 headline zh 3.83/en 7.06 —— rescued wav 仍带 NaN 哼鸣起手，**NaN 修复版全集 re-eval 应全面优于它**（待批，8 卡 30 分钟；注意 re-eval 必须从带修复的分支跑，主树采样器仍是老代码）。
